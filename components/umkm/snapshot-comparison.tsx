...
export function SnapshotComparison({ baseline, latest, previous }: { baseline: MonitoringSnapshot; latest?: MonitoringSnapshot; previous?: MonitoringSnapshot }) {
  if (!latest) return <section className="mt-5 rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center"><h2 className="font-semibold">Perbandingan sebelum & sesudah</h2><p className="mt-2 text-sm text-slate-500">Belum ada monitoring. Kondisi terkini masih sama dengan data awal (baseline).</p></section>;
